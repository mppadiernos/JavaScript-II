...
let fullNames = [];
let  nameRunners = runners.forEach(function(name) {
...
let firstNamesAllCaps = runners.map(function(firstName){
  let str = firstName.first_name;
  return str.toUpperCase(firstName.first_name);
});
...
let shirtSize = runners.filter(function (size) {
  if(size === 'large') {
    return true;
  }
});
